...
var RegExpRouter = (_f = class {
  constructor() {
    ...
    __publicField(this, "match", match$1);
...
function getStationById(id) {
  ...
  if (!station) throw new Error(`Station with id ${id} not found`);
  return station;
}
...
const FIXED_SCHEDULES = [
  { hour: 6, minute: 0, busType: "프리미엄", status: "ON_SALE" },
  { hour: 8, minute: 0, busType: "우등", status: "ON_SALE" },
  { hour: 10, minute: 0, busType: "일반", status: "ON_SALE" },
  { hour: 12, minute: 0, busType: "프리미엄", status: "SOLD_OUT" },
  { hour: 14, minute: 0, busType: "우등", status: "ON_SALE" },
  { hour: 16, minute: 0, busType: "일반", status: "ON_SALE" },
  { hour: 18, minute: 30, busType: "프리미엄", status: "ON_SALE" },
  { hour: 21, minute: 0, busType: "우등", status: "SOLD_OUT" }
];
function formatToISOWithMillis(date) {
...
function createTicket(params) {
  ...
  let departureTime = addDays(now, daysFromNow);
  departureTime = setHours(departureTime, departureHour);
  departureTime = setMinutes(departureTime, departureMinute);
  departureTime = setSeconds(departureTime, 0);
  departureTime = setMilliseconds(departureTime, 0);
  const arrivalTime = addMinutes(departureTime, durationMinutes);
  return {
    id,
    departureTime: formatToISOWithMillis(departureTime),
    arrivalTime: formatToISOWithMillis(arrivalTime),
    busNumber,
    status,
...
function generateAllRouteSchedules() {
  const schedules = [];
  const totalStations = stations.length;
  for (let departureIndex = 0; departureIndex < totalStations; departureIndex++) {
    for (let arrivalIndex = 0; arrivalIndex < totalStations; arrivalIndex++) {
      if (departureIndex === arrivalIndex) continue;
      const departureStationId = stations[departureIndex].id;
      const arrivalStationId = stations[arrivalIndex].id;
...
function generateMonthlyTickets(daysInMonth = 30) {
  const tickets2 = [];
  const routeSchedules = generateAllRouteSchedules();
  let ticketId = 1;
  for (const route of routeSchedules) {
    const departureStation = getStationById(route.departureStationId);
    const arrivalStation = getStationById(route.arrivalStationId);
      ...
      for (const schedule of route.schedules) {
        tickets2.push(
          createTicket({
            ...
            status: schedule.status || "ON_SALE"
          })
        );
      }
    }
...
const tickets = generateMonthlyTickets();
...
function hasId(item) {
  return typeof item === "object" && item !== null && "id" in item && (typeof item.id === "number" || typeof item.id === "string");
}
const repository = {
  getAll(entityName) {
  ...
  getById(entityName, id) {
    return db[entityName].find((item) => hasId(item) && item.id === id);
  },
  create(entityName, data) {
    const collection = db[entityName];
    const newId = collection.reduce((maxId, item) => {
      if (hasId(item) && typeof item.id === "number" && item.id > maxId) {
        return item.id;
      }
      return maxId;
    }, 0) + 1;
    const dataWithId = { ...data, id: newId };
    db[entityName].push(dataWithId);
    return dataWithId;
  },
  update(entityName, id, data) {
    const collection = db[entityName];
    const index = collection.findIndex((item) => hasId(item) && item.id === id);
    if (index === -1) return void 0;
    db[entityName][index] = { ...collection[index], ...data };
  ...
  find(entityName, predicate) {
    return db[entityName].filter(predicate);
  }
};
function getStationByTicketId(ticketId, isArrival) {
  const ticket = tickets.find((t) => t.id === ticketId);
  if (!ticket) return void 0;
  const stationId = isArrival ? ticket.arrivalStationId : ticket.departureStationId;
  return stations.find((s) => s.id === stationId);
}
const StationSchema = objectType({
  id: numberType(),
  name: stringType()
});
const TicketSchema = objectType({
  id: numberType(),
  departureTime: stringType(),
  arrivalTime: stringType(),
  busNumber: stringType(),
  departureStationId: numberType(),
  arrivalStationId: numberType(),
  status: enumType(["ON_SALE", "SOLD_OUT"])
});
const PassengerCountSchema = objectType({
  adults: numberType().int().min(0),
  children: numberType().int().min(0),
  newborn: numberType().int().min(0)
});
enumType(["ROUND", "ONE_WAY"]);
const SeatsSchema = arrayType(numberType().int().positive());
const BaseReservationRequestSchema = objectType({
  count: PassengerCountSchema
});
const OneWayReservationRequestSchema = BaseReservationRequestSchema.extend({
  type: literalType("ONE_WAY"),
  ticketId: numberType().int().positive(),
  seats: SeatsSchema
});
const RoundReservationRequestSchema = BaseReservationRequestSchema.extend({
  type: literalType("ROUND"),
  outboundTicketId: numberType().int().positive(),
  inboundTicketId: numberType().int().positive(),
  outboundSeats: SeatsSchema,
  inboundSeats: SeatsSchema
});
const ReservationRequestSchema = discriminatedUnionType("type", [
  OneWayReservationRequestSchema,
  RoundReservationRequestSchema
]);
const BaseReservationCreateSchema = objectType({
  departureStation: StationSchema,
  arrivalStation: StationSchema,
  count: PassengerCountSchema
});
const OneWayReservationCreateSchema = BaseReservationCreateSchema.extend({
  type: literalType("ONE_WAY"),
  ticket: TicketSchema,
  seats: SeatsSchema
});
const RoundReservationCreateSchema = BaseReservationCreateSchema.extend({
  type: literalType("ROUND"),
...
const ReservationCreateSchema = discriminatedUnionType("type", [
  OneWayReservationCreateSchema,
  RoundReservationCreateSchema
]);
const OneWayReservationResponseSchema = OneWayReservationCreateSchema.extend({
  id: numberType()
});
const RoundReservationResponseSchema = RoundReservationCreateSchema.extend({
  id: numberType()
});
const ReservationResponseSchema = discriminatedUnionType("type", [
  OneWayReservationResponseSchema,
  RoundReservationResponseSchema
]);
const TicketQueryParamsSchema = objectType({
  departureStationId: stringType().regex(/^\d+$/, "숫자여야 합니다").transform(Number).optional(),
  arrivalStationId: stringType().regex(/^\d+$/, "숫자여야 합니다").transform(Number).optional(),
  departureTime: stringType().regex(/^\d{4}-\d{2}-\d{2}$/, "YYYY-MM-DD 형식이어야 합니다").optional()
});
class DomainError extends Error {
  constructor(message2, statusCode = 400) {
    super(message2);
...
const DomainErrors = {
  stationNotFound: (stationId) => new DomainError(`터미널 정보를 찾을 수 없습니다. (ID: ${stationId})`, 404),
  outboundTicketNotFound: (ticketId) => new DomainError(`가는편 티켓을 찾을 수 없습니다. (ID: ${ticketId})`, 404),
  inboundTicketNotFound: (ticketId) => new DomainError(`오는편 티켓을 찾을 수 없습니다. (ID: ${ticketId})`, 404),
  ticketSoldOut: (ticketId) => new DomainError(`해당 티켓은 매진되었습니다. (ID: ${ticketId})`),
  sameStation: () => new DomainError("출발 터미널과 도착 터미널이 같을 수 없습니다."),
  invalidPassengerCount: () => new DomainError("최소 1명 이상의 승객이 필요합니다."),
  reservationNotFound: (reservationId) => new DomainError(`예약을 찾을 수 없습니다. (ID: ${reservationId})`, 404),
  invalidReservationId: () => new DomainError("예약 ID는 유효한 숫자여야 합니다.")
};
objectType({
  status: numberType(),
  message: stringType(),
...
function createErrorResponse(status, message2, detail) {
  return { status, message: message2, detail };
}
function toErrorResponse(error) {
  if (error instanceof RequestValidationError) {
    const firstError = error.zodError.errors[0];
    const fieldName = firstError.path.join(".");
    const message2 = `요청 데이터가 올바르지 않습니다. ${fieldName}: ${firstError.message}`;
    return createErrorResponse(400, message2);
  }
  ...
  if (error instanceof ZodError) {
    ...
    return createErrorResponse(500, "서버 내부 오류가 발생했습니다.", details);
  }
  if (error instanceof Error) {
    console.error("Unexpected error:", error);
    return createErrorResponse(500, "서버 내부 오류가 발생했습니다.", error.message);
  }
  console.error("Unknown error:", error);
  return createErrorResponse(500, "알 수 없는 오류가 발생했습니다.");
}
const getApiStations = async (c) => {
  try {
    const keyword = c.req.query("keyword");
    const stations2 = keyword ? repository.find("stations", (station) => station.name.includes(keyword)) : repository.getAll("stations");
    const parseResult = arrayType(StationSchema).safeParse(stations2);
    if (!parseResult.success) {
      return errorResponse(c, toErrorResponse(parseResult.error));
    }
    return c.json(parseResult.data);
  } catch (err) {
    return errorResponse(c, toErrorResponse(err));
  }
};
const getApiTickets = async (c) => {
  try {
    let queryParams;
    try {
      const rawParams = {
        departureStationId: c.req.query("departureStationId"),
        arrivalStationId: c.req.query("arrivalStationId"),
        departureTime: c.req.query("departureTime")
      };
      queryParams = TicketQueryParamsSchema.parse(rawParams);
    } catch (err) {
    ...
    const { departureStationId, arrivalStationId, departureTime } = queryParams;
    if (departureStationId != null && arrivalStationId != null && departureStationId === arrivalStationId) {
      throw DomainErrors.sameStation();
    }
    const tickets2 = repository.find("tickets", (ticket) => {
      if (departureStationId != null && ticket.departureStationId !== departureStationId) {
        return false;
      }
      if (arrivalStationId != null && ticket.arrivalStationId !== arrivalStationId) {
        return false;
      }
      if (departureTime != null) {
        const ticketDate = format$1(new Date(ticket.departureTime), "yyyy-MM-dd");
        if (ticketDate !== departureTime) return false;
      }
      return true;
    });
    const parseResult = arrayType(TicketSchema).safeParse(tickets2);
    if (!parseResult.success) {
      return errorResponse(c, toErrorResponse(parseResult.error));
    }
    return c.json(parseResult.data);
  } catch (error) {
    return errorResponse(c, toErrorResponse(error));
  }
};
const postApiReservations = async (c) => {
  try {
    let body;
    try {
      body = ReservationRequestSchema.parse(await c.req.json());
    } catch (err) {
      if (err instanceof ZodError) {
        throw new RequestValidationError(err);
      }
    ...
    const totalPassengers = body.count.adults + body.count.children + body.count.newborn;
    if (totalPassengers === 0) {
      throw DomainErrors.invalidPassengerCount();
    }
    const reservationResponseData = body.type === "ONE_WAY" ? processOneWayReservation(body) : processRoundReservation(body);
    const reservationCreate = ReservationCreateSchema.parse(reservationResponseData);
    const createdReservation = repository.create("reservation_responses", reservationCreate);
    return c.json({ reservationId: createdReservation.id });
  } catch (err) {
    return errorResponse(c, toErrorResponse(err));
  }
};
function processOneWayReservation(body) {
  const ticket = repository.getById("tickets", body.ticketId);
  if (!ticket) {
    throw DomainErrors.outboundTicketNotFound(body.ticketId);
  }
  if (ticket.status === "SOLD_OUT") {
    throw DomainErrors.ticketSoldOut(body.ticketId);
  }
  const departureStation = getStationByTicketId(ticket.id, false);
  const arrivalStation = getStationByTicketId(ticket.id, true);
  if (!departureStation) {
    throw DomainErrors.stationNotFound(ticket.departureStationId);
  }
  if (!arrivalStation) {
    throw DomainErrors.stationNotFound(ticket.arrivalStationId);
  }
  return {
    type: "ONE_WAY",
    departureStation,
...
function processRoundReservation(body) {
  const outboundTicket = repository.getById("tickets", body.outboundTicketId);
  const inboundTicket = repository.getById("tickets", body.inboundTicketId);
  if (!outboundTicket) {
    throw DomainErrors.outboundTicketNotFound(body.outboundTicketId);
  }
  if (outboundTicket.status === "SOLD_OUT") {
    throw DomainErrors.ticketSoldOut(body.outboundTicketId);
  }
  if (!inboundTicket) {
    throw DomainErrors.inboundTicketNotFound(body.inboundTicketId);
  }
  if (inboundTicket.status === "SOLD_OUT") {
    throw DomainErrors.ticketSoldOut(body.inboundTicketId);
  }
  const departureStation = getStationByTicketId(outboundTicket.id, false);
  const arrivalStation = getStationByTicketId(outboundTicket.id, true);
  if (!departureStation) {
    throw DomainErrors.stationNotFound(outboundTicket.departureStationId);
  }
  if (!arrivalStation) {
    throw DomainErrors.stationNotFound(outboundTicket.arrivalStationId);
  }
  return {
    type: "ROUND",
    departureStation,
    ...
    outboundTicket,
    inboundTicket,
    outboundSeats: body.outboundSeats,
    inboundSeats: body.inboundSeats,
    count: body.count
  };
}
const getApiReservationsId = async (c) => {
  try {
    const idParam = c.req.param("id");
    const id = parseInt(idParam, 10);
    if (isNaN(id)) {
      throw DomainErrors.invalidReservationId();
    }
    const reservation = repository.getById("reservation_responses", id);
    if (!reservation) {
      throw DomainErrors.reservationNotFound(id);
    }
    const validatedReservation = ReservationResponseSchema.parse(reservation);
    return c.json(validatedReservation);
  } catch (err) {
    return errorResponse(c, toErrorResponse(err));
  }
};
const server = new Hono();
server.get("/api/stations", getApiStations);
server.get("/api/tickets", getApiTickets);
server.post("/api/reservations", postApiReservations);
server.get("/api/reservations/:id", getApiReservationsId);
export {
  server as default
};
